...
class WordBiClassify(BaseModel):
    ...
    def predict(self, words):
        ...
        prob = self.bi_classification_model.predict(word_vec)
...
def main():
    context = ['请问', '杭州第一人民医院']
    word_bi_classify = WordBiClassify(cfg_path=global_conf.cfg_path,
                                      model_section='NAMED_ENTITY_RECOGNITION_MODEL_TO_BI_CLASSIFY')
    t0 = time.time()
    for i in range(1):
        results = word_bi_classify.predict(context)
    t1 = time.time()
    print(results)
    print(t1-t0)
