...
def convert_single_example_to_feature(example, max_seq_length, tokenizer):
...
def val(model, processor, args, label_list, tokenizer, device):
    ...
    f1 = np.mean(metrics.f1_score(gt, predict,average=None))
    print('f-score', f1)
...
def main():
    ...
    processors = {
        "mrpc": ClassifyProcessor,
        "mnli": MnliProcessor,
        'similar': SimilarProcessor
    }
    ...
    # print(task_name, processors)
    if task_name not in list(processors.keys()):
        raise ValueError("Task not found: %s" % (task_name))

    processor = processors[task_name]() ## 类ClassifyProcessor
    label_list = processor.get_labels()
    num_labels = len(label_list)
    # bert_model  (可以是一个文件夹路径，文件夹中必须要包括，vocab.txt,pytorch_model.bin,bert_config.json)
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if f1 > best_score:
                ...
                # 由于老的版本里面 BertTokenizer，没有save_vocabulary函数，因此这里将其单独写一遍，当有的时候，就可以直接用了
                index = 0
    ...
    #             logits = model(input_ids, segment_ids, input_mask)
    #
    #         # print('logits的类型', type(logits)) #<class 'torch.Tensor'>
    #         logits = logits.detach().cpu().numpy()
    #         label_ids = label_ids.to('cpu').numpy()
