...
def run_test():
    ...
    for line in input:
        ...
        sex = line[3].decode('utf-8')
        age = line[4].decode('utf-8')
        # classify_result_b = None
        try:
            # classify_result_b = tc_b.query_sex_age(content, sex)
            classify_result_b = tc_b.query({'q': content.encode('utf-8'), 'sex': sex.encode('utf-8'),
...
def run_test_new(output, confidence=0, level=1):
    ...
    for line in input:
        ...
        content = line[0].decode('utf-8')
        dept_name = line[1].decode('utf-8')
        age = line[3].decode('utf-8')
        sex = line[4].decode('utf-8')
        if len(line) > 2:
            second_dept_name = line[2].decode('utf-8')
            # classify_result_b = tc_b.query_sex_age(content, sex, age)
            classify_result_b = tc_b.query({'q': content.encode('utf-8'), 'sex': sex.encode('utf-8'),
                                            'age': age.encode('utf-8')},
                                           service='dept_classify')
            format = '%s==%s==%s==%s'
            ...
            if pre_dept_name != 'unknow':
                ...
                if pre_dept_name_pro >= confidence:
                    ...
                    if pre_dept_name == dept_name:
                        ...
                        if level == 2 and len(second_dept_name) > 0 and pre_dept_name == second_dept_name:
                            count2 = count2 + 1
                        buf = format % (content, dept_name, pre_dept_name, pre_dept_name_pro)
                        output2.write(buf.encode('utf-8'))
                        output2.write('\n')
