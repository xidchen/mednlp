                    ...
                    if disease_name in step_disease:
                        ...
                        if step == 1:
                            ...
                            if not physical_exam:
                                physical_exam = ''
                            content = ','.join([
                                disease_name.strip().replace(',', '，'),
                                chief_complaint.strip().replace(',', '，'),
                                medical_history.strip().replace(',', '，'),
                                past_medical_history.strip().replace(',', '，'),
                                physical_exam.strip().replace(',', '，'),
                                inspection.strip().replace(',', '，'),
                                str(sex).strip(),
                                str(int(age/365)).strip()
                            ])
                            print(content)
                            continue
...
if __name__ == '__main__':
    ...
    parser.add_option('-d', '--disease', dest='disease',
                      help='disease to test', metavar='FILE')
