...
# @File:intelligence_service_char.py
...
class CharModel(BaseTrainer):
    def __init__(self, file_number=2970812):
        super(CharModel, self).__init__(model_name='similarity_faq_char')
    ...
    def train_generator_model(self, train_file, x_test, y_test, model):
        ...
        """
        model.compile(loss="categorical_crossentropy", metrics=["categorical_accuracy"], optimizer="adam")
        model.fit_generator(data_loader.generate_arrays_from_file(train_file, batch_size, num=37, classes=self.classes),
                            steps_per_epoch=int(self.file_number/batch_size), epochs=num_epoches, max_q_size=1,
                            validation_data=(x_test, y_test), callbacks=[self.model_checkpoint])
        return model
...
if __name__ == '__main__':
    ...
    if increase_model == 1:
        ...
        model = char_model.train_model(model, trainfile)
