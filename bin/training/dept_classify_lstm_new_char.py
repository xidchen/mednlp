...
# -*- coding: utf-8 -*-
"""
the dept_classify by rnn training
"""
...
from keras.layers import Dense, Dropout, Bidirectional, TimeDistributed, Convolution1D, concatenate,\
...
class CharModel():
    ...
    def lstm_s_model(self):
        ...
        model = Sequential()
        model.add(Embedding(n_words, 32))
        model.add(Bidirectional(LSTM(128, dropout=0.5, recurrent_dropout=0.05, return_sequences=True)))
        model.add(Bidirectional(LSTM(128, dropout=0.5, recurrent_dropout=0.1, return_sequences=True)))
        model.add(Bidirectional(LSTM(128, dropout=0.5, recurrent_dropout=0.2)))
        model.add(Dense(classes, activation='softmax'))
        return model
    ...
    def lstm_cnn_s(self):
        """
        嵌入层+lstm+conv+pooling+conv+pooling+conv+pooling
        :return: model
        """
        input_a = Input(shape=(sequence_length,))
        e_result = Embedding(max_nb_words, embedding_dim, input_length=sequence_length,
    ...
    def lstm_cnn_s2(self):
        """
        嵌入层+两层lstm+conv+pooling+conv+pooling+conv+pooling
        :return: model
        """
        input_a = Input(shape=(sequence_length,))
        e_result = Embedding(max_nb_words, embedding_dim, input_length=sequence_length,
                             name="embedding")(input_a)
        b_result = Bidirectional(LSTM(128, return_sequences=True, dropout=0.5, recurrent_dropout=0.1))(e_result)
        b_result = Bidirectional(LSTM(128, return_sequences=True, dropout=0.5, recurrent_dropout=0.2))(b_result)
        conv_x1 = Convolution1D(filters=64, kernel_size=2, padding='valid', activation='relu')(b_result)
        conv_x1 = MaxPooling1D(pool_size=2)(conv_x1)
        conv_x2 = Convolution1D(filters=64, kernel_size=2, padding='valid', activation='relu')(conv_x1)
        conv_x2 = MaxPooling1D(pool_size=2)(conv_x2)
...
if __name__ == '__main__':
    ...
    print(file_number)
    char_model = CharModel(file_number=file_number)
    if modeltype == 1:
        model = char_model.lstm_cnn_s()
    elif modeltype == 2:
        model = char_model.lstm_cnn_s2()
    elif modeltype == 3:
        model = char_model.lstm_s_model()
    elif modeltype == 4:
        model = char_model.lstm_attention()
    elif modeltype == 5:
        model = char_model.lstm_attention_s()
    else:
        model = char_model.origin_model()
    x_test, y_test = data_loader.get_test_data(input_test_path, num=100)
    if increase_model == 1:
        print('increate')
        model = char_model.train_generate_increate(trainfile, x_test, y_test)
    else:
        model = char_model.train_generator_model(trainfile, x_test, y_test, model)
    char_model.save_model(model, version=version)
