...
class TCMSyndromeCharModel(BaseTrainer):
    def __init__(self, file_number=2970861):
        super(TCMSyndromeCharModel, self).__init__(model_name='tcm_diagnose.p')
        self.file_number = file_number
        self.classes = 32
        self.epoch_num = 100
        self.char_num = 6000
        self.disease_num = 10
        self.syndrome_num = 21
        self.seq_num = 600
        ...
        prescription2vector = Dept2Vector(global_conf.tcm_prescription_path)
        self.prescription_to_id = prescription2vector.name2index

    def origin_model(self):
        """  """
        disease_input = Input(shape=(self.seq_num,))
        syndrome_input = Input(shape=(self.seq_num,))
        symptom_input = Input(shape=(self.seq_num,))
        disease_embedding = Embedding(self.disease_num, 4)(disease_input)
        syndrome_embedding = Embedding(self.syndrome_num, 4)(syndrome_input)
        symptom_embedding = Embedding(self.char_num, 16)(symptom_input)
        embedding = concatenate([disease_embedding, syndrome_embedding, symptom_embedding])

        lstm2 = Bidirectional(LSTM(128, dropout=0.2, recurrent_dropout=0.02))(embedding)
        out = Dense(self.classes, activation='sigmoid')(lstm2)
        model = Model(input=[disease_input, syndrome_input, symptom_input], outputs=out)

        return model

    def make_train_input(self, file_name):
        diseases, symptoms, syndrome, prescription = [], [], [], []
        y = []
        with codecs.open(file_name, 'r', 'utf-8') as f:
            for line in f.readlines():
                items = line.strip().split('\t')
                diseases.append([self.disease_to_id[items[3]]])
                syndrome.append([self.syndrome_to_id[items[4]]])
                symptoms.append(self.char2vector.get_char_vector(items[5]))
                prescription.append([self.prescription_to_id[i] for i in items[0].split(',')])

        multilabel_binarizer = MultiLabelBinarizer()
        multilabel_binarizer.fit(prescription)
        y = multilabel_binarizer.transform(prescription)
        print(y.shape)
        print(y[0])

        x2 = pad_sequences(symptoms, maxlen=self.seq_num)
        x0 = pad_sequences(diseases, maxlen=self.seq_num)
        x1 = pad_sequences(syndrome, maxlen=self.seq_num)

        train_num = int(len(syndrome) * 0.9)

        train_x1 = x1[:train_num]
        train_x2 = x2[:train_num]
        train_x0 = x0[:train_num]
        train_y = y[:train_num]
        valid_x2 = x2[train_num:]
        valid_x1 = x1[train_num:]
        valid_x0 = x0[train_num:]
        valid_y = y[train_num:]

        return ([train_x0, train_x1, train_x2], train_y), ([valid_x0, valid_x1, valid_x2], valid_y)
